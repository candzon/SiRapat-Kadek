...
@section('content')
    <div class="container mx-auto px-4">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold">List Rapat</h1>
        </div>
                    ...
                    @if (auth()->user()->role === 'admin')
                        <div>
                            ...
                            <select id="assigned-filter" name="user_id" onchange="handleStatusChange()"
                                class="shadow border rounded py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                                <option value="All">All Peserta</option>
            ...
            <table id="workOrderTable" class="min-w-full table-auto display">
                <thead>
                    <tr class="bg-gray-200 text-black-600 uppercase text-sm leading-normal">
                        ...
                        <th class="py-3 px-6 text-left">Invited</th>
                        <th class="py-3 px-6 text-left">Judul Rapat</th>
                    ...
                    @foreach ($workOrders as $workorder)
                        <tr class="border-b border-gray-200 hover:bg-gray-100">
                            <td class="py-3 px-6 text-left whitespace-nowrap">
                                <span class="font-medium">{{ $rowNumber++ }}</span>
                            </td>
                            <td class="py-3 px-6 text-left">{{ $workorder->invited }}</td>
                            <td class="py-3 px-6 text-left">{{ $workorder->judul_rapat }}</td>
                            <td class="py-3 px-6 text-left">{{ $workorder->deskripsi }}</td>
                            <td class="py-3 px-6 text-left">{{ $workorder->tanggal }}</td>
                            <td class="py-3 px-6 text-left">{{ $workorder->waktu_mulai }}</td>
                            <td class="py-3 px-6 text-left">{{ $workorder->waktu_selesai }}</td>
                            <td class="py-3 px-6 text-left">{{ $workorder->lokasi }}</td>
                            <td class="py-3 px-6 text-left">{{ $workorder->status }}</td>
                            <td class="py-3 px-6 text-left">{{ $workorder->created_by }}</td>
