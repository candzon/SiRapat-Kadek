...
class WorkOrderController extends Controller
{
    ...
    public function store(Request $request)
    {
        try {
            $validated = $request->validate([
                'user_id' => ['required', 'int'], // Inviting participant select field
            ...
            if ($request->ajax()) {
                return response()->json([
                    'success' => false,
                    'message' => 'Gagal membuat rapat: ' . $e->getMessage()
                ], 422);
            }

            return redirect()->back()
                ->withInput()
                ->with('error', 'Gagal membuat rapat: ' . $e->getMessage());
        }
    }
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            $validated = $request->validate([
            'user_id' => ['required', 'int'], // Inviting field
            'judul_rapat' => ['required', 'string'],
            'deskripsi' => ['nullable', 'string'],
            'tanggal' => ['required', 'date'],
            ...
            return response()->json(['success' => true, 'message' => 'Rapat berhasil diupdate.']);

        } catch (\Exception $e) {
            return response()->json(['success' => false, 'message' => 'Gagal mengupdate rapat: ' . $e->getMessage()], 422);
        }
    }
